package top.wjstar.xiaohashu.auth.runner;
...
import top.wjstar.xiaohashu.auth.domain.mapper.PermissionDOMapper;
import top.wjstar.xiaohashu.auth.domain.mapper.RoleDOMapper;
import top.wjstar.xiaohashu.auth.domain.mapper.RolePermissionDOMapper;
...
public class PushRolePermissions2RedisRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        try {
            // 是否能够同步数据: 原子操作,只有在键 PUSH_PERMISSION_FLAG 不存在时,才会设置该键值为 "1",并设置过期时间为 1 天
            ...
            if (CollUtil.isNotEmpty(roleDOS)) {
                ...
                roleKeyPermissionsMap.forEach((roleKey, permissionDOList) -> {
                    String key = RedisKeyConstants.buildRolePermissionsKey(roleKey);
                    redisTemplate.opsForValue().set(key, JsonUtils.toJsonString(permissionDOList));
                });
            }

            log.info("==> 服务结束,成功同步角色权限数据到 Redis 中...");
        } catch (Exception e) {
